...
export class TTSController extends EventTarget {
  ...
  ttsKokoroClient: TTSClient | null = null;
  ...
  async init() {
    ...
    // Initialize Kokoro offline TTS (Rust ONNX backend)
    if (this.ttsKokoroClient && (await this.ttsKokoroClient.init())) {
  ...
  async setVoice(voiceId: string, lang: string) {
    ...
    if (useKokoroTTS) {
      if (!this.ttsKokoroClient) {
        throw new Error('Kokoro TTS client is not available');
      }
